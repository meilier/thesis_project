...
def run_epoch(net, epoch, id):
        ...
        #torch.save(net.state_dict(), os.getcwd() + "/cifar-simple-save/" + "cifar" +str(epoch) +".pth")

train_set = CIFAR10('./cifardata', train=True, transform=data_tf, download=True) # 重新载入数据集，申明定义的数据变换
test_set = CIFAR10('./cifardata', train=False, transform=data_tf, download=True)
...
# global simple PS
ps_dict = torch.load(os.getcwd()+"/cifar0.pth")
...
for i in range(10):
    ...
    new_net.load_state_dict(torch.load(os.getcwd()+"/cifar0.pth"))
...
for i in range(120):
    for j in range(10):
        ...
        ps_dict = partial_max_changed_cifar(tmp_ps_dict,current_dict, 0.1)
